...
import { fetchUpcomingAppointments, getAppointmentPaymentDetails } from "../../api/appointments";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        const bookings = response?.returnvalue?.response || [];

        const items = bookings.map((b) => {
          const [day, monthStr, yearAndTime] = b.start_time.split("-");
          ...
          const dt = new Date(Number(year), monthMap[monthStr], Number(day), hh, mm, ss);
          return { ...b, dt };
        });
  ...
  const handleBookFirstSession = () => navigate("/appointments/book");
  ...
  const showPayNowButton = isZohoBooking && hasPaymentPending;
    ...
    <div className={styles.dashboard}>
                ...
                <div className={styles.appointmentCard}>
                  <div className={styles.appointmentInfo}>
                    ...
                    <div className={styles.appointmentDetails}>
                      ...
                      <p>
                        👩‍⚕️{" "}
                        {upcoming?.doctor?.name || assignedDoctor?.name || "Your consultant"}
                      </p>
                      <p>📅 {upcoming.date}</p>
                      <p>🕐 {upcoming.time}</p>
                      {hasPaymentPending && (
                  ...
                  <div className={styles.appointmentActions}>
                    ...
                    {showPayNowButton && (
                      <PaymentHandler
                        ...
                        onSuccess={() => setSuccess("Payment completed successfully!")}
                        onFailure={(err) => setError(`Payment failed: ${err.message}`)}
                      />
                    )}
                  </div>
                </div>
      ...
      <section className={styles.quickActions}>
        ...
        <div className={styles.actionGrid}>
          <button className={styles.actionCard} onClick={() => navigate("/appointments/book")}>
            <span className={styles.actionIcon}>📅</span>
            <span>Book Session</span>
          </button>
          <button className={styles.actionCard} onClick={() => navigate("/appointments")}>
            <span className={styles.actionIcon}>📋</span>
            <span>View History</span>
          </button>
          <button className={styles.actionCard} onClick={() => navigate("/contact-us")}>
            <span className={styles.actionIcon}>💬</span>
      ...
      {showRescheduleModal && upcoming && assignedDoctor && (
        <RescheduleModal
...
// import React, { useEffect, useState } from "react";
// import { useNavigate } from "react-router-dom";
// import { fetchMyAppointments, getAppointmentPaymentDetails, fetchUpcomingAppointments } from "../../api/appointments";
// import { getDoctor } from "../../api/doctors";
// import RescheduleModal from "../../components/RescheduleModal";
// import PaymentHandler from "../../components/PaymentHandler";
// import styles from "./Dashboard.module.css";
